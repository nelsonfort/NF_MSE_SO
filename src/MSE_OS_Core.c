...
void os_Init(void)  {
	...
	//--------------------Inicializo el contador de ticks del sistema----------------------------
	control_OS.tick_counter = 0;
...
static void scheduler(void)  {
	...
	for(i=0;i<=prioridad_actual;i++){
		...
		//-- Además se verifican si todas la tareas dentro de esa determinada prioridad se encuentran
		//-- bloquedas para reducir el tiempo de ejecución del planificador.
		contador = ( control_OS.reg_priorityCnt & ( 0xF << (i*4) ) ) >> (i*4);

		if( (contador != 0) && (contador != ((control_OS.blocked_cnt >> (i*4)) & 0xF)) ){
...
void SysTick_Handler(void)  {
	...
	control_OS.tick_counter++;

	//-- Con Systick se actualizan los temporizadores de las tareas Bloqueadas, sòlo
	//-- si la api que lo bloqueó depende del tiempo.
...
/*************************************************************************************************
	 *  @brief Devuelve un entero sin signo de 32 bits que guarda el contador de ticks
	 *
     *
     *  @details
     *   Cada vez que se ejecuta una interrupciòn del Systick se incrementa el contador tick_counter
     *   mediante esta funciòn se puede obtener dicho valor.
     *	 Nota: esta variable puede hacer overflow por lo cual cada 49.7 dias aproximadamente
     *	 el valor obtenido comenzará de cero nuevamente. (para un systick de 1ms)
	 *  @param 		None
	 *  @return     valor actual del tick counter
***************************************************************************************************/
uint32_t os_getTickCounter(void){
	return control_OS.tick_counter;
}
...
/*************************************************************************************************
	 *  @brief Se obtiene el valor de un contador de cantidad de tareas para una dada prioridad.
     *
     *  @details
     *  Se hace la operación lògica necesaria para obtener el valor del contador dentro del registro
     *  de contadores.
     *
	 *  @param 		prioridad del contador que se desea obtener.
	 *  @return     valor del contador.
***************************************************************************************************/
uint8_t os_getRegBlockedCnt(uint8_t priority){
	return ( control_OS.blocked_cnt >> (priority*4) ) & 0xF;
}


/*************************************************************************************************
	 *  @brief Modifica el valor de un determinado contador en base a la prioridad seleccionada.
     *
     *  @details
     *   En base a la prioridad seleccionada y el valor numerico que se desea actualizar, se
     *   realiza la operación lògica para ingresar el nuevo valor al registro de contadores de
     *   tareas bloqueadas.
	 *  @param1 		Nuevo valor del contador.
	 *  @param2 		Prioridad a la que se le modifica el contador.
	 *  @return     estado del OS.
***************************************************************************************************/
void os_setRegBlockedCnt(uint8_t cnt,uint8_t priority){
	//-- Borro el valor del contador anterior
	control_OS.blocked_cnt = (~(0xF<<(priority*4))) & control_OS.blocked_cnt;

	//-- Ingreso el nuevo numero en el espacio borrado.
	control_OS.blocked_cnt = (cnt << (priority*4)) | control_OS.blocked_cnt;
}
/*************************************************************************************************
